...
class BertSimilarity:
    def __init__(self, batch_size=128, vocab_file='', max_seq_len=32, output_dir='', config_name='',
                 data_dir='', num_train_epochs=10, ckpt_name='', learning_rate=0.00005, gpu_memory_fraction=0.9):
        self.mode = None
    ...
    def model_fn_builder(self, bert_config, num_labels, init_checkpoint, learning_rate,
        ...
        def model_fn(features, labels, mode, params):  # pylint: disable=unused-argument
            ...
            (total_loss, per_example_loss, logits, probabilities) = BertSimilarity.create_model(
    ...
    def train(self):
        ...
        self.file_based_convert_examples_to_features(train_examples, label_list, self.max_seq_len, self.tokenizer,
                                                     train_file)
        tf.logging.info("***** Running training *****")
        tf.logging.info("  Num examples = %d", len(train_examples))
        tf.logging.info("  Batch size = %d", self.batch_size)
        tf.logging.info("  Num steps = %d", num_train_steps)
        train_input_fn = self.file_based_input_fn_builder(input_file=train_file, seq_length=self.max_seq_len,
                                                          is_training=True,
                                                          drop_remainder=True)
    ...
    def eval(self):
        ...
        self.file_based_convert_examples_to_features(
            eval_examples, label_list, self.max_seq_len, self.tokenizer, eval_file)
